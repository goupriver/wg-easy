...
module.exports = class WireGuard {
  ...
  async __buildConfig() {
    this.__configPromise = Promise.resolve().then(async () => {
      ...
      try {
        ...
        const address_v6 = WG_DEFAULT_ADDRESS_V6 ? WG_DEFAULT_ADDRESS_V6.replace('x', '1') : null;
  ...
  async __saveConfig(config) {
    const addresses = (config.server.addressV6) 
      ? `${config.server.address}/24, ${config.server.addressV6}/64` 
      : `${config.server.address}/24`;
    let result = `
...
}AllowedIPs = ${client.address[0]}/32${client.address[1] ? client.address[1]/128 + ', ' : ''}`;
}
...
Address = ${client.address[1] ? client.address[0] + '/24' +  ', ' + client.address[1] + '/64' : ''}
  ...
  async createClient({ name, expiredDate }) {
    ...
    for (let i = 2; i < 255; i++) {
      const client = Object.values(config.clients).find((client) => {
        return WG_DEFAULT_ADDRESS_V6 ? client.address = [WG_DEFAULT_ADDRESS.replace('x', i), WG_DEFAULT_ADDRESS_V6.replace('x', i)] : client.address === WG_DEFAULT_ADDRESS.replace('x', i);
      });

      if (!client) {
        WG_DEFAULT_ADDRESS_V6 ? address = [WG_DEFAULT_ADDRESS.replace('x', i), WG_DEFAULT_ADDRESS_V6.replace('x', i)] : address === WG_DEFAULT_ADDRESS.replace('x', i);
        break;
      }
    }
